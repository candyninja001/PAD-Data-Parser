...
127: convert('column_change', {'columns': (slice(None),lambda x: [{'index':i if i < 3 else i-6,'orbs':binary_con(orbs)} for indices,orbs in zip(x[::2],x[1::2]) for i in binary_con(indices)])}), # 0 1 2 -3 -2 -1
128: convert('row_change', {'rows': (slice(None),lambda x: [{'index':i if i < 2 else i-5,'orbs':binary_con(orbs)} for indices,orbs in zip(x[::2],x[1::2]) for i in binary_con(indices)])}), # 0 1 -3 -2 -1
...
def orb_spawn(arguments):
    return convert('spawn_orbs', {k:(arguments[k] if k in arguments else v) for k,v in orb_spawn_backups.items()})
...
17:  convert('attack_boost_1', {'un': (0,cc), 'duration': (1,cc), 'multiplier': (2,multi)}), ### look into this later
18:  convert('attack_boost_2', {'duration': (0,cc), 'multiplier': (1,multi)}), ### look into this later
19:  convert('attack_boost_3', {'un': (0,cc), 'duration': (1,cc), 'multiplier': (2,multi)}), ### look into this later
...
64:  orb_spawn({'damage': (0,multi), 'amount': (1,cc), 'orbs': [7], 'exclude_orbs': [7]}),
...
76:  convert('column_change', {'damage': 0.0, 'columns': (slice(None),lambda x: [{'index':i if i < 3 else i-6,'orbs':binary_con(orbs)} for indices,orbs in zip(x[::2],x[1::2]) for i in binary_con(indices)])}), # order 0 1 2 -3 -2 -1
77:  convert('column_change', {'damage': (6,multi), 'columns': (slice(0,6),lambda x: [{'index':i if i < 3 else i-6,'orbs':binary_con(orbs)} for indices,orbs in zip(x[::2],x[1::2]) for i in binary_con(indices)])}),
78:  convert('row_change', {'damage': 0.0, 'rows': (slice(None),lambda x: [{'index':i if i < 2 else i-5,'orbs':binary_con(orbs)} for indices,orbs in zip(x[::2],x[1::2]) for i in binary_con(indices)])}),       # order 0 1 -3 -2 -1
79:  convert('row_change', {'damage': (6,multi), 'rows': (slice(0,6),lambda x: [{'index':i if i < 2 else i-5,'orbs':binary_con(orbs)} for indices,orbs in zip(x[::2],x[1::2]) for i in binary_con(indices)])}),
80:  unimplimented(80),
